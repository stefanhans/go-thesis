...
func handlePublisherRequest(conn net.Conn) {
	...
	case chatgroup.Message_MEMBERLIST_REQUEST:

		log.Printf("MEMBERLIST_REQUEST: %v\n", msg)
	...
	case chatgroup.Message_CMD_LIST_REQUEST:

		log.Printf("CMD_LIST_REQUEST: %v\n", msg)
...
func handleSubscribe(message *chatgroup.Message, addr net.Addr) error {
	...
	err := publishDisplayerRequest(message, chatgroup.Message_SUBSCRIBE_REPLY)
	if err != nil {
...
func handleUnsubscribe(message *chatgroup.Message) error {
	...
	err := publishDisplayerRequest(message, chatgroup.Message_UNSUBSCRIBE_REPLY)
	if err != nil {
...
func handlePublish(message *chatgroup.Message, addr net.Addr) error {
	...
	err := publishDisplayerRequest(message, chatgroup.Message_PUBLISH_REPLY)
	if err != nil {
...
func handleCmdMemberList(message *chatgroup.Message, addr net.Addr) error {
	...
	log.Printf("Memberlist request from %v: %q\n", message.Sender.Name, message.MemberList)
	...
	err = replyMemberlistRequest(message)
	if err != nil {
...
func replyCmdRequest(message *chatgroup.Message) error {
	...
	message.MsgType = chatgroup.Message_CMD_LIST_REPLY
...
func replyMemberlistRequest(message *chatgroup.Message) error {

	message.MsgType = chatgroup.Message_MEMBERLIST_REPLY

	err := sendDisplayerRequest(message, message.Sender.Ip+":"+message.Sender.Port)
	if err != nil {
		return fmt.Errorf("could not send memberlist reply: %v", err)
	}
